...
module.exports = {
  name: "allvcunmute",
  helpname : "allvcunmute <vocal> ",
  aliases: ["vcunmuteall"] ,

  description: "Permet de unmute tous les utilisateur dans une voc",
  /**
   * @param {Client} client
  ...
  run: async (client, message, args) => {
    ...
    message.channel.send({embeds:
     [ new MessageEmbed().setTitle(
        `Tous les utilisateur dans \`${channel.name}\` ont été unmute`
      )  .setColor(color)
    ...
    let logs = message.guild.channels.cache.get(db.get(`logmod_${message.guild.id}`))
            if(logs) {
              logs.send({embeds : [new MessageEmbed()
                  ...
                  .setDescription(`${message.member.user.username} a **unmute tout les utilisateur dans  ${channel} **`)
                  .setColor(color)
                  .setFooter(`Prefix actuel: ${prefix}  • Owls Bots`)
